...
export function PageHeader({
  ...
  return (
    <header
      className={cn('gap-default flex flex-wrap items-start justify-between', className)}
      data-testid={dataTestId}
    >
      <div className="min-w-0">
        ...
        {back && 'smart' in back ? (
          ...
          <Link
            href={back.href}
            className="text-content-muted hover:text-content-emphasis text-xs transition-colors duration-150 ease-out"
            data-testid="page-header-back"
          >
            ← {back.label}
          </Link>
        ) : null}
        {eyebrow && <Eyebrow data-testid="page-header-eyebrow">{eyebrow}</Eyebrow>}
        ...
        {description !== undefined && description !== null && (
          <Caption data-testid="page-header-description">{description}</Caption>
        )}
        {meta && (
          <div
            className="gap-tight mt-1 flex flex-wrap items-center"
