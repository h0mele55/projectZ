...
export function BreadcrumbsProvider({ children }: { children: ReactNode }) {
  ...
  const [stack, setStack] = useState<ReadonlyArray<ReadonlyArray<BreadcrumbItem>>>([]);

  const push = useCallback((items: ReadonlyArray<BreadcrumbItem>) => {
    const entry = items;
  ...
  const api = useMemo<BreadcrumbsApi>(() => ({ items, push }), [items, push]);
 ...
 */
export function useBreadcrumbs(items: ReadonlyArray<BreadcrumbItem> | undefined): void {
  const ctx = useContext(BreadcrumbsContext);
