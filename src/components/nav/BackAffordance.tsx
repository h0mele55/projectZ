...
import { usePreviousPath, tenantSlugFromPath } from '@/lib/nav/usePreviousPath';
import { resolveCanonicalParent, type CanonicalParent } from '@/lib/nav/canonical-parents';
...
export function BackAffordance({ override, noFallback }: BackAffordanceProps) {
  ...
  if (override) {
    ...
    const canonical = noFallback ? null : resolveCanonicalParent(pathname, tenantSlug);
    ...
    if (referrer && referrer !== pathname && !referrerIsSibling) {
      destination = {
        href: referrer,
        label: labelFromPathname(referrer, sectionLabels, t('ui.previousPage')),
      };
    } else {
  ...
  return (
    <Link
      ...
      className="text-content-muted hover:text-content-emphasis focus-visible:ring-ring-default inline-flex items-center gap-1 rounded-sm text-xs focus-visible:ring-2 focus-visible:outline-none motion-safe:transition-colors motion-safe:duration-150 motion-safe:ease-out print:hidden"
