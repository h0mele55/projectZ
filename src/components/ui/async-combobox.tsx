...
interface AsyncBaseProps<TMeta> {
   ...
   */
  onSearch: (query: string, signal: AbortSignal) => Promise<AsyncOption<TMeta>[]>;
...
export type AsyncComboboxProps<TMeta = unknown> =
  AsyncSingleProps<TMeta> | AsyncMultipleProps<TMeta>;
...
export function useAsyncSearch<TMeta>(
  ...
  const [options, setOptions] = React.useState<AsyncOption<TMeta>[]>(initial ?? []);
...
export function AsyncCombobox<TMeta = unknown>(props: AsyncComboboxProps<TMeta>) {
  ...
  const [selectedCache, setSelectedCache] = React.useState<Map<string, AsyncOption<TMeta>>>(
    new Map(),
  );

  const cacheOption = React.useCallback((opt: AsyncOption<TMeta> | null | undefined) => {
    if (!opt) return;
  ...
  const selected: AsyncOption<TMeta> | null = value ? (selectedCache.get(value) ?? null) : null;
