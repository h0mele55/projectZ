        ...
        {visible.map((entry, idx) => {
          if (entry === 'ellipsis') {
            return (
              <li
                key={`ellipsis-${idx}`}
                className="text-content-muted flex items-center gap-1"
                aria-hidden="true"
              >
                <span>…</span>
          ...
          return (
            <li key={`crumb-${entry.index}`} className="flex min-w-0 items-center gap-1">
              {item.href && !isCurrent ? (
                <Link
                  href={item.href}
                  className="text-content-muted hover:text-content-emphasis max-w-trunc-loose truncate transition"
                ...
                <span
                  className={cn(
                    'max-w-trunc-loose truncate',
                    isCurrent ? 'text-content-emphasis font-medium' : 'text-content-muted',
                  )}
              ...
              {showSeparator && <SeparatorView>{separator}</SeparatorView>}
            </li>
          );
        })}
      </ol>
 ...
 */
function collapse(items: ReadonlyArray<BreadcrumbItem>, maxVisible: number): CollapsedEntry[] {
...
function SeparatorView({ children }: { children: React.ReactNode }) {
  return (
    <span className="text-content-subtle select-none" aria-hidden="true">
      {children}
    </span>
  );
}
