...
interface ChartFrameProps<T> {
   ...
   */
  children: (args: { width: number; height: number; data: T }) => ReactNode;
...
export function ChartFrame<T>({
  ...
  if (!mounted || state.kind === 'loading') {
    return (
      <ChartFrameOuter
        ...
        stateKind="loading"
      >
        {loadingFallback ?? <Skeleton className="size-full rounded-lg" />}
      </ChartFrameOuter>
    );
  }
  ...
  if (state.kind === 'error') {
    return (
      <ChartFrameOuter
        ...
        stateKind="error"
      >
        {errorFallback ?? <ErrorState title={t('couldntLoadChart')} description={state.message} />}
      </ChartFrameOuter>
    );
  }
  ...
  if (state.kind === 'empty') {
    return (
      ...
      >
        {emptyFallback ?? (
          <EmptyState size="sm" title={t('noDataYet')} description={t('emptyDescription')} />
        )}
      </ChartFrameOuter>
    );
  }
  ...
  return (
    <div
      ...
      className={cn(
        ...
        'bg-bg-elevated/40 relative rounded-lg',
        className,
      )}
