...
export function ChartGloss({
  ...
  return (
    <linearGradient id={id} x1={x1} y1={y1} x2={x2} y2={y2}>
      ...
      <stop offset="45%" stopColor="#ffffff" stopOpacity={peak * 0.15} />
...
export function chartGlossId(chartId: string, seriesIndex?: number): string {
  return seriesIndex === undefined ? `${chartId}-gloss` : `${chartId}-gloss-${seriesIndex}`;
}
...
export const ChartSheenSweep = forwardRef<SVGLinearGradientElement, ChartSheenSweepProps>(
  function ChartSheenSweep({ id, direction = 'horizontal' }, ref): ReactElement {
