...
export function ChartLinearGradient({
  ...
  return (
    <linearGradient id={id} x1={x1} y1={y1} x2={x2} y2={y2}>
      <stop offset="0%" stopColor={start} stopOpacity={opacity ?? 1} />
 ...
 */
export const ChartFlowGradient = forwardRef<SVGLinearGradientElement, ChartFlowGradientProps>(
  function ChartFlowGradient({ id, series, direction = 'horizontal' }, ref): ReactElement {
    const { x1, y1, x2, y2 } = directionToVector(direction);
    const { start, end } = seriesStops(series);
