...
export type ChartLegendProps = ChartLegendSeriesProps | ChartLegendGradientProps;
 ...
 */
export function seriesDotBackground(entry: ChartLegendSeriesEntry): string | undefined {
...
function ChartLegendSeries({ series, className }: ChartLegendSeriesProps): ReactElement {
  return (
    <ul
      className={cn(
        'gap-x-default text-content-muted flex flex-wrap items-center gap-y-1 text-xs',
        className,
      )}
      data-chart-legend
    ...
    >
      {series.map((entry, i) => (
        <li key={`${entry.name}-${i}`} className="inline-flex items-center gap-1.5">
          <span
            aria-hidden="true"
...
function ChartLegendGradient({
  ...
  return (
    <div
      className={cn('text-content-muted inline-flex flex-col gap-1 text-xs', className)}
      data-chart-legend
      data-chart-legend-variant="gradient"
    >
      {label && <span className="text-content-default font-medium">{label}</span>}
      <svg role="img" aria-label={ariaLabel} width={160} height={10} className="overflow-visible">
        <defs>
          <linearGradient id={heatScale.gradientId} x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stopColor={`var(${heatScale.startVar})`} />
            <stop offset="100%" stopColor={`var(${heatScale.endVar})`} />
          </linearGradient>
        </defs>
        <rect x={0} y={0} width={160} height={10} rx={3} fill={`url(#${heatScale.gradientId})`} />
      </svg>
