 ...
 * portable contract.
 */
export function useChartFlow({ active, distance, direction = 'horizontal' }: UseChartFlowArgs) {
 ...
 */
export function useChartSheen({ distance, direction = 'horizontal' }: UseChartSheenArgs) {
  const ref = useRef<SVGLinearGradientElement | null>(null);
  const reduced = useReducedMotion();
  ...
  useEffect(() => {
    ...
    const tick = (t: number) => {
      ...
      // appear / disappear.
      const phase = (elapsed % CHART_SHEEN_PERIOD_MS) / CHART_SHEEN_PERIOD_MS;
