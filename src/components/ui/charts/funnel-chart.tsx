    ...
    <div className="relative">
      ...
      >
        ...
        {steps.map(({ id, value, colorClassName, seriesIndex }, idx) => {
          ...
          const deltaPct = prev !== null && prev > 0 ? (value / prev) * 100 : null;
          return (
            <Fragment key={id}>
              {/* Background */}
              {tooltips && (
                  ...
                  {({ path }) => {
                    ...
                    return (
                      <motion.path
                        ...
                        className={cn(!seriesIndex && colorClassName, 'pointer-events-none')}
                        fill={
                          seriesIndex ? `url(#${chartId}-series-${seriesIndex})` : 'currentColor'
                        }
                      />
                    );
                  }}
          ...
          <ChartTooltipContainer title={tooltipStep.label}>
            <div className="flex flex-wrap justify-between gap-x-4 gap-y-2">
              <div className="gap-tight flex items-center">
                ...
                />
                <p className="text-content-muted whitespace-nowrap capitalize">
                  {formatPercentage((tooltipStep.value / maxValue) * 100) + '%'}
                </p>
              </div>
              <p className="text-content-emphasis font-medium whitespace-nowrap">
  ...
  return (
    <g>
      ...
      <Text
        ...
        fill="currentColor"
        fontSize={14}
        className={cn('pointer-events-none font-medium brightness-50 select-none', colorClassName)}
      >
        {value}
      </Text>
    </g>
