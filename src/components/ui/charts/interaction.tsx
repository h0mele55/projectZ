...
import type { Data, Datum, TimeSeriesDatum } from './types';
 ...
 */
export function useChartHover<T extends Datum = Datum>(data: Data<T>): ChartHoverState<T> {
 ...
 * the surface.
 */
export function ChartTooltipContainer({ children, className, title }: ChartTooltipContainerProps) {
  return (
    <div
      data-chart-tooltip
      className={cn(
        'pointer-events-none',
        'border-border-default bg-bg-elevated rounded-lg border shadow-lg',
        'px-3 py-2',
        'text-content-default text-xs leading-snug',
        'max-w-xs min-w-[8rem]',
        className,
      )}
    >
      {title ? (
        <div className="text-content-muted mb-1.5 text-[11px] font-semibold tracking-wider uppercase">
          {title}
        </div>
      ) : null}
...
export function ChartTooltipRow({ label, value, swatch, className }: ChartTooltipRowProps) {
  const swatchIsClass = typeof swatch === 'string' && swatch.startsWith('bg-');
  return (
    <div
      data-chart-tooltip-row
      className={cn('gap-default flex items-center justify-between', className)}
    >
      <span className="text-content-muted inline-flex items-center gap-1.5">
        {swatch ? (
          swatchIsClass ? (
            <span aria-hidden="true" className={cn('size-2 rounded-full', swatch)} />
          ) : (
            <span
              aria-hidden="true"
      ...
      <span className="text-content-emphasis font-medium tabular-nums">{value}</span>
    </div>
  );
}
