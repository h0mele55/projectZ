...
import type { ChartMargin, ChartPadding, Data, Datum, Series } from './types';
...
export function getDateExtent<T extends Datum>(
  data: Data<T>,
): {
  startDate: Date;
  endDate: Date;
...
export function buildYScale({
  ...
  return scaleLinear<number>({
    domain: [minY - range * (padding.bottom ?? 0), maxY + range * (padding.top ?? 0)],
    range: [height, 0],
    nice: true,
    clamp: true,
  });
 ...
 */
export function pickXAxisTickValues<T extends Datum>(data: Data<T>, maxTicks: number): Date[] {
