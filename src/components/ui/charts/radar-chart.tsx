...
import { ChartRadialGradient, chartGradientId, type ChartSeriesIndex } from './chart-gradient';
...
export function RadarChart({
  ...
  return (
    <ChartFrame state={state} className={className} testId={testId} emptyFallback={emptyFallback}>
      {({ width, height, data }) => (
        <RadarChartInner
          width={width}
          height={height}
...
function RadarChartInner({
  ...
  const cx = width / 2;
  const cy = height / 2;
  const outerRadius = Math.max(0, Math.min(width, height) / 2 - DEFAULT_INSET);
  ...
  const polygonPath =
    points.map((p, i) => `${i === 0 ? 'M' : 'L'} ${p.valueX} ${p.valueY}`).join(' ') + ' Z';
  ...
  return (
    <svg width={width} height={height} role="img" aria-label={ariaLabel ?? 'Radar chart'}>
      <defs>
        <ChartRadialGradient
          id={fillGradId}
          series={seriesIndex}
        ...
        {points.map((p) => {
          ...
          return (
            <Line
              ...
              stroke={isHovered ? `var(--chart-series-${seriesIndex}-end)` : 'var(--border-subtle)'}
          ...
          return (
            ...
            >
              <Text
                ...
                fill={isHovered ? 'var(--content-emphasis)' : 'var(--content-muted)'}
                style={{
