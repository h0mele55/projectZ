  ...
  return (
    <ParentSize className={props.className ?? 'relative'}>
      {({ width, height }) => {
        ...
        return <TimeSeriesChartInner {...props} width={width} height={height} />;
      }}
    </ParentSize>
  );
}
...
function TimeSeriesChartInner<T extends Datum>({
  ...
  const { yScale, xScale } = useMemo(
    ...
    [startDate, endDate, minY, maxY, height, width, data, type, padding.top, padding.bottom],
  );
                  ...
                  {series
                    ...
                    .map((s) => (
                      <Circle
                        ...
                        r={4}
                        className={s.colorClassName ?? 'text-brand-emphasis'}
                        fill="currentColor"
                      />
                    ))}
                </>
        ...
        <div className="pointer-events-none absolute inset-0">
          {tooltipData && (
            <TooltipWrapper
              ...
              offsetLeft={isBandScale ? xScale.bandwidth() * (1 + xScale.padding()) : 8}
              offsetTop={12}
              className="absolute"
              unstyled={true}
            >
