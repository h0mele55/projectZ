...
export function buildHeatColorMix(seriesIndex: ChartSeriesIndex, intensity: number): string {
...
export function buildStepValues(domain: [number, number], steps: number): number[] {
  const [min, max] = domain;
  const out: number[] = [];
...
export function useHeatScale(opts: HeatScaleOptions): HeatScale {
  const { domain, series = 1, steps = 5, range = [0.15, 1], idPrefix = 'heat' } = opts;
  return useMemo<HeatScale>(() => {
    ...
    return {
      colorFor: (value: number) => buildHeatColorMix(series, clampIntensity(value, domain, range)),
      intensityFor: (value: number) => clampIntensity(value, domain, range),
      gradientId,
      startVar,
      endVar,
      stepValues,
  ...
  }, [domain, series, steps, range, idPrefix]);
}
