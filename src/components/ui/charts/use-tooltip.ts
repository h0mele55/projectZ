...
export function useTooltip<T extends Datum>({
  ...
  const defaultTooltipDatum = defaultIndex !== undefined ? data[defaultIndex] : undefined;
    ...
    () =>
      defaultTooltipDatum !== undefined
        ? {
            ...
            tooltipTop: snapToY
              ? yScale(series.find((s) => s.id === seriesId)!.valueAccessor(defaultTooltipDatum))
              : 0,
          }
        : undefined,
  ...
  const scheduleTooltipUpdate = useCallback(
    (next: { tooltipData: TimeSeriesDatum<T>; tooltipLeft: number; tooltipTop: number }) => {
  ...
  useEffect(() => {
    if (
      ...
      visxTooltip.tooltipData?.date.getTime() !== tooltipSyncContext.tooltipDate.getTime()
    ) {
      const d = data.find((d) => d.date.getTime() === tooltipSyncContext.tooltipDate?.getTime());
      if (!d) return;
    ...
    } else if (tooltipSyncContext.tooltipDate === null && visxTooltip.tooltipData?.date) {
      visxTooltip.hideTooltip();
      onHoverDateChange?.(null);
    }
  }, [tooltipSyncContext.tooltipDate, visxTooltip.tooltipData, snapToX, snapToY, xScale, yScale]);
  ...
  const handleTooltip = useCallback(
    (event: React.TouchEvent<SVGRectElement> | React.MouseEvent<SVGRectElement>) => {
      ...
      const x0 =
        'invert' in xScale
          ? xScale.invert(x)
          : (xScale.domain()[Math.round((x - xScale.step() * 0.75) / xScale.step())] as
              Date | undefined);
  ...
  return {
    ...
    hideTooltip: () => {
      ...
      defaultTooltipData ? visxTooltip.showTooltip(defaultTooltipData) : visxTooltip.hideTooltip();
      onHoverDateChange?.(null);
    },
  };
}
