 ...
 */
export function reorderOrder(order: ReadonlyArray<string>, fromId: string, toId: string): string[] {
  if (fromId === toId || !order.includes(fromId) || !order.includes(toId)) {
...
export function isModifiedFromDefault(
  ...
  return order.length !== defaults.length || order.some((id, i) => id !== defaults[i]);
}
