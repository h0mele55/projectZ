...
export { COMBOBOX_DEFAULT_MESSAGES, getComboboxMessages, type ComboboxMessages } from './messages';
...
export function Combobox<TMultiple extends boolean | undefined = undefined, TMeta = unknown>({
  multiple,
  ...
  const [sortedOptions, setSortedOptions] = useState<ComboboxOption<TMeta>[] | undefined>(
    undefined,
  );
  ...
  const handleSelect = (option: ComboboxOption<TMeta>) => {
    ...
    if (!isAlreadySelected && maxSelected && selected.length >= maxSelected) return;
    ...
    if (isMultiple) {
      ...
      (setSelected as ((opt: ComboboxOption<TMeta> | null) => void) | undefined)?.(
  ...
  const sortOptions = useCallback(
    (opts: ComboboxOption<TMeta>[], searchText: string) =>
      searchText === ''
        ? [
            ...opts.filter((o) => o.first && !selected.some((s) => s.value === o.value)),
            ...selected,
            ...opts.filter((o) => !o.first && !selected.some((s) => s.value === o.value)),
          ]
        : opts,
  ...
  const createOptionItem = (
    <Command.Item
      className={cn(
        ...
        'text-content-default gap-compact flex cursor-pointer items-center rounded-md px-3 py-2 text-left text-sm whitespace-normal',
  ...
  const selectedTriggerText = selected
      ...
      : (trigger ?? (
          <Button
            ...
            className={cn(
              ...
              'gap-tight flex',
              effectiveInvalid && 'border-border-error focus-visible:ring-border-error',
            )}
            ...
            text={
              <>
                <div className={cn('min-w-0 grow truncate text-left', labelProps?.className)}>
            ...
            icon={
              IconProp ? (
                isReactNode(IconProp) ? (
                  ...
                  <IconProp className={cn('size-4 shrink-0', iconProps?.className)} />
                )
              ) : undefined
            }
          />
    ...
    <>
      ...
      <Popover
        ...
        popoverContentClassName={cn(
          ...
          matchTriggerWidth && 'sm:min-w-[var(--radix-popover-trigger-width)]',
            ...
            >
              {!hideSearch && (
                <div className="border-border-subtle flex items-center overflow-hidden rounded-t-lg border-b">
                  <Command.Input
                    ...
                    className={cn(
                      'text-content-emphasis placeholder:text-content-muted grow border-0 bg-transparent py-3 pr-2 pl-4 outline-none focus:ring-0 sm:text-sm',
                      inputClassName,
                    )}
                    onKeyDown={(e) => {
              ...
              >
                <Command.List className={cn('flex w-full min-w-[100px] flex-col gap-1 p-1')}>
                  {showLoading ? (
                    <Command.Loading>
                      <div className="flex h-12 items-center justify-center" data-combobox-loading>
                        <LoadingSpinner />
                      </div>
                    </Command.Loading>
                  ) : sortedOptions!.length > COMBOBOX_VIRTUALIZE_THRESHOLD ? (
  ...
  return (
    <>
      <DisabledTooltip disabledTooltip={option.disabledTooltip}>
        <Command.Item
          className={cn(
            'gap-compact flex cursor-pointer items-center rounded-md px-3 py-2 text-left text-sm',
            ...
            hasDescription ? 'py-2.5 whitespace-normal' : 'whitespace-normal',
            'data-[selected=true]:bg-bg-subtle',
            Boolean(disabled || option.disabledTooltip) && 'cursor-not-allowed opacity-50',
            className,
          )}
        ...
        >
          ...
          <div
            className={cn(
              'gap-tight flex min-w-0 grow items-center',
              hasDescription && 'flex-col items-start gap-0.5',
            )}
          >
            {option.icon && (
              <span className="text-content-default shrink-0">
                {isReactNode(option.icon) ? option.icon : <option.icon className="h-4 w-4" />}
              </span>
            )}
            <span
              className={cn(
                'grow break-words',
                hasDescription ? 'text-content-emphasis' : 'text-content-default',
              )}
            >
              {option.label}
            </span>
            {hasDescription && <span className="text-content-subtle text-sm">{description}</span>}
          </div>
          {right}
          {!multiple && selected && <Check2 className="text-content-default size-4 shrink-0" />}
        </Command.Item>
      </DisabledTooltip>
      {option.separatorAfter && <Command.Separator className="bg-border-subtle -mx-1 my-1 h-px" />}
    </>
  );
}
...
const Empty = forwardRef<HTMLDivElement, HTMLProps<HTMLDivElement>>((props, forwardedRef) => {
  ...
  return (
    <div ref={forwardedRef} cmdk-empty="" role="presentation" data-combobox-empty {...props} />
  );
});
