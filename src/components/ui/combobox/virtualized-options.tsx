...
import { Check2, CheckboxCheckedFill, CheckboxUnchecked, type Icon } from '../icons';
...
export function VirtualizedComboboxOptions<TMeta>({
  ...
  const selectedSet = React.useMemo(() => new Set(selected.map((o) => o.value)), [selected]);
  ...
  const getItemSize = React.useCallback(
    (index: number) => {
      ...
      return Math.max(h, hasDescription ? TWO_LINE_OPTION_HEIGHT : SINGLE_LINE_OPTION_HEIGHT);
    },
  ...
  for (let i = 0; i < optionsLength && viewportHeight < MAX_VIEWPORT_PX; i++) {
    viewportHeight += getItemSize(i);
  }
  viewportHeight = Math.min(MAX_VIEWPORT_PX, Math.max(getItemSize(0), viewportHeight));
      ...
      <VirtualizedList
        ...
        renderItem={({ index, style }) => {
          ...
          const isDisabled = Boolean(
            (!isSelected && typeof maxSelected === 'number' && selected.length >= maxSelected) ||
              ...
              <DisabledTooltip disabledTooltip={option.disabledTooltip}>
                <div
                  ...
                  className={cn(
                    ...
                    description ? 'py-2.5 whitespace-normal' : 'whitespace-normal',
                    ...
                    {option.icon && (
                      <span className="text-content-default shrink-0">
                        {isReactNode(option.icon)
                          ...
                          : React.createElement(option.icon as Icon, { className: 'h-4 w-4' })}
                      </span>
                    )}
                    <span
                      className={cn(
                        'grow break-words',
                        description ? 'text-content-emphasis' : 'text-content-default',
                      )}
                    >
                      {option.label}
                    </span>
