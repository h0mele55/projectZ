...
export interface CopyButtonProps extends VariantProps<typeof copyButtonVariants> {
...
export const CopyButton = forwardRef<HTMLButtonElement, CopyButtonProps>(function CopyButton(
  ...
  const handleClick = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (ok) {
      onCopy?.(value);
      toast.success(successMessage ?? t('labelCopied', { label: resolvedLabel }));
    } else {
  ...
  const button = (
    ...
    >
      {copied ? (
        <Check className={cn(iconSize, 'text-content-success')} aria-hidden="true" />
      ) : (
