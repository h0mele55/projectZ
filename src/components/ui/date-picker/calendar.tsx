...
import { forwardRef, useState, type ElementType, type HTMLAttributes } from 'react';
  ...
  ({ onClick, icon: Icon, disabled, ...props }, forwardedRef) => {
    ...
    const button = (
      <button
        ...
        disabled={disabled}
        className={cn(
          'flex size-7 shrink-0 items-center justify-center rounded-md border p-1 select-none',
          'transition-colors duration-150 outline-none',
          'border-border-subtle text-content-muted',
          'hover:bg-bg-muted hover:text-content-emphasis',
          'active:bg-bg-subtle',
          'focus-visible:ring-ring focus-visible:ring-offset-bg-default focus-visible:ring-2 focus-visible:ring-offset-2',
          'disabled:text-content-subtle disabled:pointer-events-none disabled:opacity-50',
        )}
        onClick={onClick}
        {...props}
      >
...
export function Calendar({
  ...
  const handleMonthChange = (nextMonth: Date) => {
    setMonth(nextMonth);
    (props as { onMonthChange?: (d: Date) => void }).onMonthChange?.(nextMonth);
  };
    ...
    <DayPicker
      ...
      components={{
        ...
        MonthCaption: ({ calendarMonth }) => {
          ...
          return (
            <div className="flex items-center justify-between" data-testid="calendar-caption">
              <div className="flex items-center gap-1">
                {showYearNavigation && !hidePreviousButton && (
                  <NavigationButton
                    disabled={
                      !canGoBack ||
                      !!(startMonth && addYears(month, -1).getTime() < startMonth.getTime())
                    }
                ...
                {!hidePreviousButton && (
                  <NavigationButton
                    ...
                    onClick={() => canGoBack && handleMonthChange(previousMonth)}
                    icon={ChevronLeft}
                  />
                )}
              </div>
              ...
              <div
                ...
                className="text-content-emphasis text-sm font-semibold capitalize tabular-nums"
              >
              ...
              <div className="flex items-center gap-1">
                {!hideNextButton && (
                  <NavigationButton
                    ...
                    onClick={() => canGoForward && handleMonthChange(nextMonth)}
                    icon={ChevronRight}
                  />
                )}
                {showYearNavigation && !hideNextButton && (
                  <NavigationButton
                    disabled={
                      !canGoForward ||
                      !!(endMonth && addYears(month, 1).getTime() > endMonth.getTime())
                    }
