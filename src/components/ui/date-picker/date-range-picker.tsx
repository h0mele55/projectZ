...
export interface DateRangePickerProps extends ControlledDateRangeProps, PickerProps {
   ...
   */
  onChange?: (next: DateRangeValue, context?: { preset?: ResolvableDateRangePreset }) => void;
}
...
export function DateRangePicker({
  ...
  const [internal, setInternal] = useState<DateRangeValue>(
    isControlled ? (value as DateRangeValue) : (defaultValue ?? { from: null, to: null }),
  );
  const committed: DateRangeValue = isControlled ? (value as DateRangeValue) : internal;
  ...
  const [draft, setDraft] = useState<RDPDateRange | undefined>(fromDateRangeValue(committed));
  const [month, setMonth] = useState<Date | undefined>(committed.to ?? committed.from ?? undefined);
  ...
  return (
    <DatePickerContext.Provider value={{ isOpen: open, setIsOpen: setOpen }}>
      <Popover
        ...
        content={
          <div className="bg-bg-default flex w-full flex-col">
            <div
              className={cn('relative flex w-full', 'flex-col sm:flex-row-reverse sm:items-start')}
            >
              <div className="min-w-0 flex-1">
                <CalendarPrimitive
                  ...
                  locale={locale}
                  classNames={{
                    months: 'flex flex-row divide-x divide-border-subtle',
                  }}
                  {...props}
                />
              </div>
              {materialisedPresets.length > 0 && (
                <div
                  className={cn(
                    'border-border-subtle w-full shrink-0 border-b',
                    'sm:w-48 sm:border-r sm:border-b-0',
            ...
            {clearable && (committed.from || committed.to) && (
              <div className="border-border-subtle flex items-center justify-end border-t px-2 py-1.5">
                <button
                  type="button"
                  onClick={handleClear}
                  className={cn(
