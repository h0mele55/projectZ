...
export function Presets<TPreset extends Preset>({
  ...
  return (
    <Command
      className={cn('w-full rounded-md focus:outline-none', className)}
      tabIndex={0}
        ...
        {presets.map((preset) => {
          ...
          const row = (
            <Command.Item
              ...
              className={cn(
                'group gap-compact flex cursor-pointer items-center justify-between',
                'rounded-md px-2.5 py-1.5 text-sm',
                'text-content-default',
                'data-[selected=true]:bg-bg-muted data-[selected=true]:text-content-emphasis',
                isActive && 'bg-bg-subtle text-content-emphasis font-medium',
              )}
          ...
          if (preset.tooltipContent) {
            return (
              <Tooltip key={preset.id} content={preset.tooltipContent} side="right">
                {row}
              </Tooltip>
            );
          }
