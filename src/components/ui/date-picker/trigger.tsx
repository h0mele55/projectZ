...
export interface TriggerProps extends ComponentProps<'button'>, VariantProps<typeof triggerStyles> {
  ...
  (
    ...
    return (
      ...
      >
        <CalendarIcon
          aria-hidden="true"
          className="text-content-muted group-hover:text-content-default size-4 shrink-0"
        />
        <span className="flex-1 overflow-hidden text-left text-ellipsis whitespace-nowrap">
          {children ? (
            <span className="text-content-emphasis" data-testid="date-picker-trigger-value">
              {children}
            </span>
          ) : placeholder ? (
            <span className="text-content-subtle" data-testid="date-picker-trigger-placeholder">
              {placeholder}
            </span>
          ) : null}
        </span>
