  ...
  return (
    <div
      className={cn(
        ...
        size === 'sm' ? 'gap-y-2 px-3 py-2' : 'gap-y-4 px-6 py-12',
        className,
      )}
    ...
    >
      <div
        className={cn(
          'border-border-subtle bg-bg-muted flex items-center justify-center rounded-lg border',
          size === 'sm' ? 'size-10' : 'size-14',
        )}
      >
        <Icon
          className={cn('text-content-muted', size === 'sm' ? 'size-4' : 'size-6')}
          aria-hidden="true"
        />
      </div>
      <p className="text-content-emphasis text-center text-base font-medium">{title}</p>
      {description && (
        <p className="text-content-muted max-w-sm text-center text-sm text-balance">
          {description}{' '}
          {learnMore && (
...
function renderAction(action: EmptyStateAction, intent: 'primary' | 'secondary') {
