 ...
 */
export function createTypedFilterDefs<_T = unknown>(): <D extends Record<string, FilterDefInput>>(
  defs: D,
) => {
  /** Filter[] for passing to Filter.Select / Filter.List. */
  filters: FilterDef[];
...
export function codecForExampleValue<V extends PersistableFilterValue>(
  ...
  switch (typeof example) {
    ...
    default:
      throw new Error(`codecForExampleValue: no built-in codec for ${typeof example}`);
  }
}
