...
import { normalizeRangeBounds, sanitizeNumericDraft, storageToDraft } from './filter-range-utils';
      ...
      <div className="flex items-center gap-1">
        <Tooltip content={t('backToFilterList')}>
          <button
            type="button"
            className={cn(
              'text-content-muted flex size-6 shrink-0 items-center justify-center rounded-md p-2',
        ...
        <span className="text-content-emphasis text-sm font-medium">{label}</span>
      </div>
...
function RangeEndControl({
  ...
  const stepNum = Number.isFinite(parsedStep) && parsedStep > 0 ? parsedStep : 1;

  return (
    <div
      className={cn(
        'border-border-subtle bg-bg-default flex h-10 min-w-0 flex-1 items-stretch overflow-hidden rounded-lg border',
        'transition-[border-color,box-shadow] duration-150 ease-out motion-reduce:transition-none',
        'focus-within:border-border-emphasis focus-within:ring-ring focus-within:ring-4',
      )}
    >
      <input
        ref={setInputRef}
        ...
        className="text-content-emphasis placeholder:text-content-subtle min-w-0 flex-1 border-0 bg-transparent px-3 py-1 text-sm outline-none focus:ring-0"
        value={draft}
        onChange={(e) => {
        ...
        onKeyDown={(e) => {
          ...
          const hasCaret = rangeStart !== null && rangeEnd !== null && rangeStart === rangeEnd;
...
function FilterRangeContent({
  ...
  return (
    <div
      className="gap-compact flex w-full flex-col p-2 sm:max-w-[282px] sm:min-w-[282px]"
      onKeyDownCapture={(e) => {
    ...
    >
      <div className="gap-tight flex items-end">
        ...
        <span className="text-content-muted shrink-0 pb-2.5 text-xs">{t('rangeTo')}</span>
...
export function FilterRangePanel({
  ...
  return (
    <>
      <FilterRangeHeader label={filter.label} onBack={onBack} onClear={onClear} />
