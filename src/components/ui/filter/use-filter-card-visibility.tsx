...
function renderFilterIcon(icon: FilterType['icon']): ReactNode {
  ...
  const isComponentType =
    typeof icon === 'function' || (typeof icon === 'object' && '$$typeof' in (icon as object));
  if (isComponentType) {
...
export function useFilterCardVisibility({
  ...
  const defaults = useMemo(() => defaultOrder(defaultVisibleDefs), [defaultVisibleDefs]);
  ...
  const order = useMemo(() => {
    ...
    if (reconciled.length === 0 && Array.isArray(stored) && stored.length > 0) {
      return defaults;
    }
    return reconciled;
  ...
  const visibleCards = useMemo(
    () => order.map((id) => cardById.get(id)).filter((c): c is CardDefinition => Boolean(c)),
    [order, cardById],
  );
  const items = useMemo(() => buildChecklistItems(cards, order), [cards, order]);
  const someModified = useMemo(() => isModifiedFromDefault(order, defaults), [order, defaults]);

  const onToggle = useCallback(
    (id: string) => setStored((prev) => toggleOrder(reconcileOrder(prev, defaultVisibleDefs), id)),
    [setStored, defaultVisibleDefs],
  );
  const onReset = useCallback(() => setStored(defaults), [setStored, defaults]);
  const onReorder = useCallback(
    (fromId: string, toId: string) =>
      setStored((prev) => reorderOrder(reconcileOrder(prev, defaultVisibleDefs), fromId, toId)),
    [setStored, defaultVisibleDefs],
  );
