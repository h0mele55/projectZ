...
const FormField = React.forwardRef<HTMLDivElement, FormFieldProps>(
  (
    ...
    const descriptionId = description && !hasError ? `${controlId}-description` : undefined;
    const errorId = hasError ? `${controlId}-error` : undefined;

    const describedBy =
      [childProps['aria-describedby'], descriptionId, errorId].filter(Boolean).join(' ') ||
      undefined;
    ...
    const injectedChild = React.cloneElement(children as React.ReactElement<InjectedControlProps>, {
      id: controlId,
    ...
    return (
      <div
        ref={ref}
        className={cn(
          isHorizontal ? 'gap-compact flex items-center' : 'flex flex-col gap-1.5',
          className,
        )}
        data-form-field
      >
        {label && (
          <div className={cn('flex items-center gap-1.5', isHorizontal && 'shrink-0')}>
        ...
        <div className={cn(isHorizontal && 'flex-1')}>
          ...
          {hasError && <FormError id={errorId}>{error}</FormError>}
        </div>
      </div>
    );
  },
