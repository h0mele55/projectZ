...
describe('useThresholdLoadMore', () => {
  it('below threshold: every row visible, hasMore false', () => {
    const { result } = renderHook(() => useThresholdLoadMore(range(30), { threshold: 50 }));
  ...
  it('above threshold: shows the first slice; hasMore true', () => {
    const { result } = renderHook(() => useThresholdLoadMore(range(120), { threshold: 50 }));
  ...
  it('loadMore reveals the next increment-sized batch', () => {
    const { result } = renderHook(() => useThresholdLoadMore(range(200), { threshold: 50 }));
  ...
  it('loadMore is a no-op when nothing more to reveal', () => {
    const { result } = renderHook(() => useThresholdLoadMore(range(10), { threshold: 50 }));
