...
export interface UseOptimisticUpdateResult<T> {
   ...
   */
  update: <R>(optimistic: T | ((prev: T) => T), commit: () => Promise<R>) => Promise<R>;
}
...
export function useOptimisticUpdate<T>(
  ...
  const update = useCallback(
    async <R>(optimistic: T | ((prev: T) => T), commit: () => Promise<R>): Promise<R> => {
      ...
      const nextOptimistic =
        typeof optimistic === 'function' ? (optimistic as (p: T) => T)(current) : optimistic;
