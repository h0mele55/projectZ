...
export type SheetRootProps = SheetRootBaseProps & VariantProps<typeof sheetWidthVariants>;
...
function SheetRoot({
  ...
  return (
    <RootComponent {...rootProps}>
      <Drawer.Portal>
        <Drawer.Overlay
          className={overlayClassName ?? 'bg-bg-overlay fixed inset-0 z-40 backdrop-blur-sm'}
          data-sheet-overlay
        />
        <Drawer.Content
          {...contentProps}
        ...
        >
          <div
            ...
            className={cn(
              ...
              'surface-popup-texture text-content-emphasis flex size-full grow flex-col overflow-hidden rounded-lg',
            )}
          >
            {fallbackTitle}
...
function Title({ className, ...rest }: ComponentProps<typeof Drawer.Title>) {
  return (
    <Drawer.Title
      className={cn('text-content-emphasis text-lg font-semibold', className)}
      {...rest}
    />
  );
}

function Description({ className, ...rest }: ComponentProps<typeof Drawer.Description>) {
  return <Drawer.Description className={cn('text-content-muted text-sm', className)} {...rest} />;
}
...
function Header({
  ...
  return (
    <div
      data-sheet-header
      className={cn(
        'gap-compact border-border-subtle flex shrink-0 items-start justify-between border-b px-5 py-4',
        className,
      )}
      {...rest}
    >
        ...
        <Tooltip content={t('close')} shortcut="Esc">
          <Drawer.Close asChild>
            <button
              ...
              aria-label={t('close')}
              data-sheet-close
              className="text-content-muted hover:bg-bg-muted hover:text-content-emphasis focus-visible:ring-ring shrink-0 rounded-md p-1.5 transition-colors focus-visible:ring-2 focus-visible:outline-none"
            >
              <X className="size-4" />
            </button>
          </Drawer.Close>
