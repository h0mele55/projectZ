...
function StatusBadge({
  ...
  const ResolvedIcon = icon !== null ? (icon ?? defaultIcons[variant ?? 'neutral']) : null;

  return (
    <DynamicTooltipWrapper tooltipProps={tooltip ? { content: tooltip } : undefined}>
      <span
        className={cn(
          statusBadgeVariants({ variant, tone, size }),
          tooltip && 'cursor-help',
          onClick &&
            'cursor-pointer transition-[filter] duration-150 select-none hover:brightness-75 hover:saturate-[1.25]',
          className,
        )}
        onClick={onClick}
        {...props}
