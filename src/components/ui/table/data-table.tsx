...
export function DataTable<T>({
  ...
  const effectiveOnRowSelectionChange =
    onRowSelectionChange ?? (hasBatchActions ? () => {} : undefined);
  const effectiveSelectedRows =
    selectedRows ?? (hasBatchActions && !hasExplicitSelection ? internalSelection : undefined);
  const effectiveSelectionControls =
    selectionControls ?? (hasBatchActions ? renderBatchActions(batchActions!) : undefined);
