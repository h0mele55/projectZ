 ...
 */
export interface BatchActionButtonProps extends ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: 'default' | 'danger';
  icon?: ReactNode;
}
  ...
  ({ variant = 'default', icon, className, children, ...props }, ref) => (
    <button
      ...
      className={cn(
        ...
        'focus-visible:ring-offset-background focus-visible:ring-2 focus-visible:ring-[var(--ring)] focus-visible:ring-offset-2 focus-visible:outline-none',
        'disabled:pointer-events-none disabled:opacity-50',
        variant === 'default' &&
          'bg-bg-elevated text-content-emphasis hover:bg-bg-muted active:bg-bg-subtle border-border-subtle border',
        variant === 'danger' &&
  ...
  return (
    <div
      className={cn(
        ...
        'bg-bg-elevated w-full border-b border-[var(--brand-default)]',
        'transition-opacity duration-100',
        selectedCount > 0 ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0',
        className,
      )}
      ...
      {...(selectedCount === 0 ? ({ inert: '' } as Record<string, string>) : {})}
      role="toolbar"
          ...
          <Tooltip content={t('table.clearSelection')} shortcut="Esc">
            <button
              ...
              className={cn(
                ...
                'rounded px-1 focus-visible:ring-2 focus-visible:ring-[var(--ring)] focus-visible:outline-none',
              )}
              onClick={(e) => {
                e.stopPropagation();
