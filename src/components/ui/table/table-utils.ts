...
export function isClickOnInteractiveChild(e: MouseEvent) {
  for (
    ...
    if (
      ['button', 'input', 'textarea'].includes(target.tagName.toLowerCase()) ||
      target.getAttribute('role') === 'dialog' ||
      target.id === 'modal-backdrop' ||
      ['data-radix-popper-content-wrapper', 'data-vaul-overlay', 'data-vaul-drawer'].some(
        (attr) => target.getAttribute(attr) !== null,
      )
    )
      return true;
  }
