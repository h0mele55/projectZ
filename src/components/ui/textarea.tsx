...
export interface TextareaProps
  extends React.TextareaHTMLAttributes<HTMLTextAreaElement>, Omit<CvaTextareaProps, 'invalid'> {
  error?: string;
  description?: string;
  invalid?: boolean;
}
...
const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(
  (
    { className, invalid, error, description, id, 'aria-describedby': ariaDescribedBy, ...props },
    ref,
  ) => {
    ...
    return (
      <div className="w-full">
        <textarea
          ...
          className={cn(textareaVariants({ invalid: effectiveInvalid }), className)}
          {...props}
        />
