...
function TooltipBody({
  ...
  return (
    <div className="flex flex-col gap-1">
      {hasHeader && (
        <div className="gap-compact flex items-center justify-between">
          {title != null && (
            <span className="text-content-emphasis text-[13px] font-semibold">{title}</span>
          )}
          {shortcut && (
            <kbd className="border-border-subtle bg-bg-subtle text-content-muted ml-auto rounded border px-1.5 py-0.5 text-[10px] font-medium">
              {shortcut}
            </kbd>
          )}
        </div>
      ...
      {children != null && (
        <div className={cn(hasHeader ? 'text-content-muted' : 'text-content-default')}>
          {children}
        </div>
      )}
    </div>
  ...
  return (
    <Tooltip {...tooltipProps}>
      <button
        ...
        className="text-content-muted hover:text-content-default focus-visible:ring-ring focus-visible:ring-offset-background inline-flex h-4 w-4 items-center justify-center rounded-full transition-colors outline-none focus-visible:ring-2 focus-visible:ring-offset-2"
      >
