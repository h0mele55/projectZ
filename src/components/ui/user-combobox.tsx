...
export function useTenantMembers(tenantSlug: string, options?: { enabled?: boolean }) {
...
function toOption(member: Member): ComboboxOption<Member> {
  return {
    ...
    icon: (
      <InitialsAvatar
        value={readableField(member.name) ?? readableField(member.email)}
        size="sm"
...
export function UserCombobox(props: UserComboboxProps) {
  ...
  const options = React.useMemo(() => filtered.map(toOption), [filtered]);

  if (props.multiple) {
    const selectedOptions = options.filter((o) => props.selectedIds.includes(o.value));
    return (
  ...
  const selected = options.find((o) => o.value === props.selectedId) ?? null;
  ...
  return (
    <Combobox<false, Member>
      ...
      selected={selected}
      setSelected={(option) =>
        props.onChange(option?.value ?? null, (option?.meta as Member | undefined) ?? null)
      }
      loading={!preloadedMembers && query.isLoading}
      placeholder={placeholder}
      searchPlaceholder={searchPlaceholder}
