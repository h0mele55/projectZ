...
interface ApiRouteHandler<Context> {
  (req: NextRequest, ctx: AsyncifyParams<Context>): Promise<NextResponse | Response>;
  (req: NextRequest, ctx: Context): Promise<NextResponse | Response>;
}
...
export function withApiErrorHandling<Context = unknown>(
  ...
  const impl = async (req: NextRequest, ctxIn: unknown): Promise<NextResponse | Response> => {
    ...
    return runWithRequestContext({ requestId, route, startTime }, async () => {
      // ── OTel root span ──
