...
let _aggCacheComputeDuration: ReturnType<ReturnType<typeof getMeter>['createHistogram']> | null =
  null;
...
let _verificationEmailFailed: ReturnType<ReturnType<typeof getMeter>['createCounter']> | null =
  null;
...
function getVerificationEmailFailed() {
  if (!_verificationEmailFailed) {
    _verificationEmailFailed = getMeter().createCounter('auth.verification_email.failed', {
      description:
        'Verification emails that the mailer rejected (operator-only signal — the register API still returns 200)',
      unit: '1',
    });
  }
...
function getSsrHit() {
  if (!_ssrHit)
    _ssrHit = getMeter().createCounter('cache.ssr.hit', {
      description: 'SSR payload cache hits',
      unit: '1',
    });
  return _ssrHit;
}
function getSsrMiss() {
  if (!_ssrMiss)
    _ssrMiss = getMeter().createCounter('cache.ssr.miss', {
      description: 'SSR payload cache misses',
      unit: '1',
    });
  return _ssrMiss;
}
function getSsrDuration() {
  if (!_ssrDuration)
    _ssrDuration = getMeter().createHistogram('cache.ssr.duration', {
      description: 'SSR payload fetch duration (hit = cache read, miss = compute)',
      unit: 'ms',
    });
  return _ssrDuration;
}
...
function getAiDecisionLogged() {
  if (!_aiDecisionLogged) {
    _aiDecisionLogged = getMeter().createCounter('ai.decision.logged', {
      description:
        'AI decisions logged, labelled by provider + feature. One per AI-feature invocation.',
      unit: '1',
    });
  }
...
function getAiDecisionGuardBlocks() {
  if (!_aiDecisionGuardBlocks) {
    _aiDecisionGuardBlocks = getMeter().createCounter('ai.decision.guard_blocks', {
      description:
        'AI decisions whose output guard redacted/dropped content, labelled by provider + feature.',
      unit: '1',
    });
  }
...
function getAiDecisionOutcomes() {
  if (!_aiDecisionOutcomes) {
    _aiDecisionOutcomes = getMeter().createCounter('ai.decision.outcome', {
      description:
        'Human-oversight outcomes on AI suggestions (accepted/edited/rejected), labelled by outcome. Acceptance rate is derivable.',
      unit: '1',
    });
  }
